//
//  SplineCurveChartExample.swift
//  GlassKit
//

import SwiftUI

// MARK: - Spline Curve Chart Card

struct SplineCurveChartCard: View {
    ...
    @State private var curveIntensity: Double = 50
    ...
    // Curve Behavior
    @State private var curveType: CurveType = .natural
    @State private var tension: Double = 50
    @State private var clampOvershoot: Bool = false
    ...
    @State private var areaOpacity: Double = 0.20
    ...
    // Smoothing
    @State private var smoothingStrength: Double = 60
    ...
    enum CurveType: String, CaseIterable { case natural = "Natural", monotone = "Monotone", basis = "Basis" }
    ...
    /// Catmull-Rom → cubic Bezier (Natural spline with tension).
    func naturalPath(points: [CGPoint]) -> Path {
        guard points.count >= 2 else { return Path() }
        let t = CGFloat(smoothingStrength / 100.0)
        var path = Path()
        ...
        for i in 1..<points.count {
            ...
            var cp1 = CGPoint(x: curr.x + (next.x - prev.x) * t / 3,
                              y: curr.y + (next.y - prev.y) * t / 3)
            var cp2 = CGPoint(x: next.x - (next2.x - curr.x) * t / 3,
                              y: next.y - (next2.y - curr.y) * t / 3)
            if clampOvershoot {
                let minY = min(curr.y, next.y)
                let maxY = max(curr.y, next.y)
                cp1.y = max(minY, min(maxY, cp1.y))
                cp2.y = max(minY, min(maxY, cp2.y))
            }
    ...
    /// Fritsch-Carlson monotone cubic spline — never overshoots, preserves monotonicity.
    func monotonePath(points: [CGPoint]) -> Path {
        guard points.count >= 2 else { return Path() }
        let n = points.count
        var m = [CGFloat](repeating: 0, count: n)

        // Compute chord slopes
        ...
        // Scale tangents by tension
        let t = CGFloat(tension / 100.0)
        var path = Path()
        path.move(to: points[0])
        for i in 0..<(n - 1) {
            let curr = points[i]
            ...
            var cp1 = CGPoint(x: curr.x + dx / 3, y: curr.y + m[i] * dx / 3 * t)
            var cp2 = CGPoint(x: next.x - dx / 3, y: next.y - m[i + 1] * dx / 3 * t)
            if clampOvershoot {
                let minY = min(curr.y, next.y)
                let maxY = max(curr.y, next.y)
                cp1.y = max(minY, min(maxY, cp1.y))
    ...
    /// Cubic uniform B-spline approximation — ultra-smooth, glides past data points.
    func basisPath(points: [CGPoint]) -> Path {
        guard points.count >= 2 else { return Path() }
        let t = CGFloat(smoothingStrength / 100.0)
        var path = Path()
        // For B-spline: control point offsets use 1/6 of the difference between neighbors
        path.move(to: points[0])
        for i in 1..<points.count {
            let prev  = i > 1 ? points[i - 2] : points[i - 1]
            let curr  = points[i - 1]
            let next  = points[i]
            let next2 = i < points.count - 1 ? points[i + 1] : points[i]
            // B-spline control vectors (1/6 factor vs 1/3 for Catmull-Rom)
            let scale = t / 6.0
            var cp1 = CGPoint(x: curr.x + (next.x - prev.x) * scale,
                              y: curr.y + (next.y - prev.y) * scale)
            var cp2 = CGPoint(x: next.x - (next2.x - curr.x) * scale,
                              y: next.y - (next2.y - curr.y) * scale)
            if clampOvershoot {
                let minY = min(curr.y, next.y)
                let maxY = max(curr.y, next.y)
                cp1.y = max(minY, min(maxY, cp1.y))
    ...
    func splinePath(points: [CGPoint]) -> Path {
        switch curveType {
        case .natural:  return naturalPath(points: points)
        case .monotone: return monotonePath(points: points)
        case .basis:    return basisPath(points: points)
        }
    }
    ...
    func areaPath(points: [CGPoint], topM: CGFloat, chartH: CGFloat) -> Path {
        ...
        var path = splinePath(points: points)
            ...
            ZStack(alignment: .topLeading) {
                ...
                if !pts.isEmpty {
                    ...
                    if areaFill && pts.count >= 2 {
                        ...
                        if gradientFill {
                            area.fill(LinearGradient(
                                colors: [lineColor.opacity(areaOpacity), lineColor.opacity(areaOpacity * 0.05)],
                    ...
                    if shadowEnabled && pts.count >= 2 {
                        splinePath(points: pts)
                            .stroke(lineColor.opacity(0.3), style: StrokeStyle(lineWidth: CGFloat(lineThickness) + 4, lineCap: .round, lineJoin: .round))
                            .blur(radius: 4)
                    }

                    // Spline line with draw animation
                    if pts.count >= 2 {
                        splinePath(points: pts)
                            .trim(from: 0, to: appeared ? 1 : 0)
                            .stroke(
                                gradientFill
                                    ? AnyShapeStyle(LinearGradient(colors: [lineColor, lineColor.opacity(0.55)], startPoint: .leading, endPoint: .trailing))
                    ...
                    if showPoints && pointSize > 0 {
                        ForEach(Array(pts.enumerated()), id: \.offset) { i, pt in
                            ...
                            let pShape = pointShape == .circle
                                ? AnyShape(Circle())
                                : AnyShape(RoundedRectangle(cornerRadius: 3, style: .continuous))
                            let clipShape = pointShape == .circle
                                ? AnyShape(Circle())
                                : AnyShape(RoundedRectangle(cornerRadius: 3, style: .continuous))

                            ZStack {
                                if pointFill {
                                    pShape.fill(lineColor).glassEffect(.clear, in: clipShape)
                                }
                                if pointStroke {
    ...
    var geometrySettingsView: some View {
        settingsSection {
            ...
            HStack(spacing: 10) {
                ...
                Slider(value: $curveIntensity, in: 0...100, step: 1)
                Text("\(Int(curveIntensity))%")
                    .font(.system(size: 11, weight: .medium, design: .rounded))
                    .foregroundStyle(.secondary).frame(width: 36, alignment: .trailing)
            }
            HStack(spacing: 10) {
    ...
    // MARK: - Settings: Curve Behavior

    var curveBehaviorSettingsView: some View {
        settingsSection {
            HStack(spacing: 10) {
                settingsLabel("Type")
                HStack(spacing: 4) {
            ...
            HStack(spacing: 10) {
                ...
                Slider(value: $tension, in: 0...100, step: 1)
                Text("\(Int(tension))%")
                    .font(.system(size: 11, weight: .medium, design: .rounded))
                    .foregroundStyle(.secondary).frame(width: 36, alignment: .trailing)
            }
            HStack(spacing: 10) {
                settingsLabel("Clamp")
                Toggle("", isOn: $clampOvershoot).labelsHidden().scaleEffect(0.8)
                Text("Clamp overshoot").font(.system(size: 11, weight: .medium, design: .rounded)).foregroundStyle(.secondary)
                Spacer()
            }
        }
    }
    ...
    var areaFillSettingsView: some View {
        settingsSection {
            HStack(spacing: 10) {
                ...
                Text("Fill area under curve").font(.system(size: 11, weight: .medium, design: .rounded)).foregroundStyle(.secondary)
                Spacer()
            }
            if areaFill {
                HStack(spacing: 10) {
    ...
    var smoothingSettingsView: some View {
        settingsSection {
            HStack(spacing: 10) {
                settingsLabel("Smoothing")
                Slider(value: $smoothingStrength, in: 0...100, step: 1)
                Text("\(Int(smoothingStrength))%")
                    .font(.system(size: 11, weight: .medium, design: .rounded))
                    .foregroundStyle(.secondary).frame(width: 36, alignment: .trailing)
            }
        }
