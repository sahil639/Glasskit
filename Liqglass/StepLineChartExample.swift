//
//  StepLineChartExample.swift
//  GlassKit
//

import SwiftUI

// MARK: - Step Line Chart Card

struct StepLineChartCard: View {
    ...
    @State private var stepType: StepType = .end
    ...
    enum StepType: String, CaseIterable { case start = "Start", middle = "Middle", end = "End" }
    ...
    /// Builds the stepped path based on stepType.
    /// - start: vertical drop first, then horizontal to next
    /// - end:   horizontal to next x, then vertical drop
    /// - middle: horizontal to midpoint, vertical, then horizontal to next
    func stepPath(points: [CGPoint]) -> Path {
        ...
        for i in 1..<points.count {
            ...
            switch stepType {
            case .start:
                // Rise/drop immediately at current x, then go right
                path.addLine(to: CGPoint(x: curr.x, y: next.y))
                path.addLine(to: next)
            case .end:
                // Go right to next x first, then rise/drop
                path.addLine(to: CGPoint(x: next.x, y: curr.y))
                path.addLine(to: next)
            case .middle:
                let midX = (curr.x + next.x) / 2
                path.addLine(to: CGPoint(x: midX, y: curr.y))
                path.addLine(to: CGPoint(x: midX, y: next.y))
                path.addLine(to: next)
            }
        }
        return path
    ...
    func areaPath(points: [CGPoint], topM: CGFloat, chartH: CGFloat) -> Path {
        ...
        var path = stepPath(points: points)
            ...
            ZStack(alignment: .topLeading) {
                ...
                if !pts.isEmpty {
                    ...
                    if shadowEnabled && pts.count >= 2 {
                        stepPath(points: pts)
                            .stroke(lineColor.opacity(0.3), style: StrokeStyle(lineWidth: CGFloat(lineThickness) + 4, lineCap: .square, lineJoin: .miter))
                            .blur(radius: 4)
                    }

                    // Step line with draw animation
                    if pts.count >= 2 {
                        stepPath(points: pts)
    ...
    var geometrySettingsView: some View {
        settingsSection {
            ...
            HStack(spacing: 10) {
                settingsLabel("Step")
                HStack(spacing: 4) {
                    ForEach(StepType.allCases, id: \.self) { s in
                        pillButton(s.rawValue, isSelected: stepType == s) {
                            withAnimation(.spring(response: 0.3, dampingFraction: 0.75)) { stepType = s }
                        }
                    }
                }
                Spacer()
    ...
    func addItem() {
        withAnimation(.spring(response: 0.4, dampingFraction: 0.8)) {
            items.append(AnalyticsDataItem(
                label: "\(items.count + 1)",
                percentage: Double.random(in: 20...90),
