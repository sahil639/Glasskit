//
//  AreaLineChartExample.swift
//  GlassKit
//

import SwiftUI

// MARK: - Area Line Chart Card

struct AreaLineChartCard: View {
    ...
    @State private var areaOpacity: Double = 0.45
    ...
    func areaPath(points: [CGPoint], topM: CGFloat, chartH: CGFloat) -> Path {
        guard points.count >= 2 else { return Path() }
        // Baseline Y position within the chart
        let baselineY = topM + chartH * (1 - yRatio(for: effectiveBaseline))
        let clampedBaselineY = min(topM + chartH, max(topM, baselineY))
        var path = linePath(points: points)
        path.addLine(to: CGPoint(x: points.last!.x, y: clampedBaselineY))
        path.addLine(to: CGPoint(x: points.first!.x, y: clampedBaselineY))
        path.closeSubpath()
        return path
    }
    ...
    var chartView: some View {
        GeometryReader { geo in
            ...
            ZStack(alignment: .topLeading) {
                ...
                // Baseline reference line (if not at chart bottom)
                if baseline != .zero || effectiveBaseline != effectiveYMin {
                    ...
                    .stroke(lineColor.opacity(0.35), style: StrokeStyle(lineWidth: 1, dash: [5, 4]))
                }
                ...
                if !pts.isEmpty {
                    ...
                    // Area fill — drawn before line so line sits on top
                    if areaFill && pts.count >= 2 {
                        let area = areaPath(points: pts, topM: topM, chartH: chartH)
                        if gradientFill {
                            area.fill(LinearGradient(
                                colors: [lineColor.opacity(areaOpacity), lineColor.opacity(areaOpacity * 0.08)],
        ...
        settingsSection {
            ...
            if areaFill {
                ...
                HStack(spacing: 10) {
                    ...
                    Text("Fade to baseline").font(.system(size: 11, weight: .medium, design: .rounded)).foregroundStyle(.secondary)
                    Spacer()
                }
            }
            HStack(spacing: 10) {
